  ...
  return (
    <div className="space-y-4">
      <form
        ...
        action={formAction}
        onSubmit={(event) => {
          const currentSelectedCount = document.querySelectorAll<HTMLInputElement>(`input[name="ids"][form="${formId}"]:checked`).length;
          ...
          if (selectedOperation?.dangerous) {
            const confirmed = window.confirm(`${selectedOperation.label}\n选中 ${currentSelectedCount} 条。\n${selectedOperation.impact}\n确认继续？`);
            if (!confirmed) {
              event.preventDefault();
            }
          }
      ...
      <DashboardCard className="px-4 py-4">
        ...
        {selectedOperation ? <p className="mt-3 text-sm leading-6 text-slate-500">{selectedOperation.impact}</p> : null}
        {disabled ? <p className="mt-3 text-sm text-amber-600">预览环境只读，请在 Windows 本地验收。</p> : null}
        {state.message ? (
          <div className={["mt-3 rounded-2xl border px-4 py-3 text-sm leading-6", state.ok ? "border-emerald-200 bg-emerald-50 text-emerald-700" : "border-amber-200 bg-amber-50 text-amber-700"].join(" ")}>
            <p>{state.message}</p>
            {state.result ? (
              <p>
