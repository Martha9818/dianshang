...
- Current task state: V1.5 Stabilization UI/AI follow-up is complete locally, verified, and ready for review.
...
- Link-import auto filters now update the URL while restoring the user's current scroll position, and API image generation retries `/v1/images/generations` when a provider root URL returns an HTML page instead of JSON.
