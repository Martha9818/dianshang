...
| V1.7.1 phase closeout status-sync patch | P2 | Synced the active current-line docs to the accepted `V1.7.1` closeout state and ignored local agent-artifact directories so the milestone can be preserved cleanly without touching product behavior. | `npx tsx scripts/thread-v17-1-thread00-stabilization-verify.mts`, `npx tsx scripts/thread-v17-1-thread01-boundary-verify.mts`, `npm run encoding:check`, `npm run typecheck`, `npm run lint`, `npm run build`, and clean tracked `git status` passed during closeout. |
